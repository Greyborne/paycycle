...
import {
  getConfig, ensureMaterialized, buildProjection, loadTemplates, effectiveAmount,
} from '../services/budget.js';
import { addDays, daysBetween, monthlyOccurrences, periodContaining, todayISO } from '../services/schedule.js';
...
// except per-user dismissals, keyed by a stable id so a dismissed instance
// stays hidden while the next occurrence (new key) shows up again.
async function computeNotifications(budget, cfg) {
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (!cfg || !req.budget.onboarding_complete) return res.json({ notifications: [] });
    await ensureMaterialized(req.budget.id, cfg);
    const all = await computeNotifications(req.budget, cfg);
    if (!all.length) return res.json({ notifications: [] });
    const { rows: dismissed } = await q(
      'SELECT key FROM notification_dismissals WHERE user_id = $1 AND key = ANY($2)',
      [req.userId, all.map((n) => n.key)]
    );
    ...
    res.json({
      notifications: all
        ...
        .sort((a, b) => order[a.severity] - order[b.severity] || (a.date < b.date ? -1 : 1)),
    });
  } catch (err) {
    next(err);
  }
